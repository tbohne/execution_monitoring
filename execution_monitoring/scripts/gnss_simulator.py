...
class GNSSSimulator:
    ...
    def __init__(self):
        ...
        rospy.Subscriber("/toggle_simulated_high_cov", String, self.toggle_high_cov_callback, queue_size=1)
        ...
        self.sim_high_cov = False

        self.gps_publisher = rospy.Publisher('/fix', NavSatFix, queue_size=1)

    def toggle_high_cov_callback(self, msg):
        self.sim_high_cov = not self.sim_high_cov
    ...
    def sim_gps_callback(self, nav_sat_fix):
        ...
        if self.sim_cov_history_failure:
            ...
            for _ in range(config.COVARIANCE_HISTORY_LENGTH):
                ...
                rospy.sleep(1)
            self.sim_cov_history_failure = False
        if self.sim_high_cov:
            nav_sat_fix.position_covariance_type = config.GNSS_COVARIANCE_TYPE_DIAGONAL_KNOWN
            nav_sat_fix.position_covariance = [0.0, 0.0, 0.0, 0.0, 55.0, 0.0, 0.0, 0.0, 0.0]
            self.sim_high_cov = False
